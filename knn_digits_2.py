...
def generate_scaled_data(data_multiplier=1, use_synthetic=False, n_samples=1000):
    ...
    if use_synthetic:
        ...
        # Use digits dataset and multiply if needed
        digits = load_digits()
        X, y = digits.data, digits.target

        if data_multiplier > 1:
            # Replicate data with slight noise to avoid exact duplicates
            X_list = [X]
            y_list = [y]
...
def main():
    ...
    if rank == 0:
        ...
        parser.add_argument(
            "--data-multiplier", type=int, default=1, help="Multiply digits dataset by this factor"
        )
    ...
    if rank == 0:
        X, y = generate_scaled_data(config["data_multiplier"], config["use_synthetic"], config["n_samples"])
        ...
        print(f"  - Data multiplier: {config['data_multiplier']}")
    ...
    # Computation phase: KNN predictions
    comp_start = MPI.Wtime()
    local_predictions = []

    for i, test_point in enumerate(local_test_chunk):
        pred = knn_predict(test_point, X_train, y_train, config["k"])
    ...
    if rank == 0:
        ...
        print("\nLoad Balance:")
        load_imbalance = (max_comp_time - min_comp_time) / max_comp_time * 100
        ...
        actual_speedup = avg_comp_time * size / max_comp_time if max_comp_time > 0 else 0
        ...
        # Save results to file for batch processing
        results_line = f"{size},{X_train.shape[0]},{X_test.shape[0]},{config['k']},{accuracy:.6f},{total_exec_time:.6f},{total_comm_time:.6f},{max_comp_time:.6f},{efficiency:.2f}\n"

        try:
            with open("knn_benchmark_results.csv", "a") as f:
                # Write header if file is new
                try:
                    with open("knn_benchmark_results.csv", "r") as check_f:
                        if len(check_f.read()) == 0:
                            f.write(
                                "processes,train_samples,test_samples,k,accuracy,total_time,comm_time,comp_time,efficiency\n"
                            )
